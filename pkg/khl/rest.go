...
func (s *Session) MessageCreate(m *MessageCreate) (resp *MessageResp, err error) {
	...
	if err != nil {
		s.Logger.Error().Caller(3).Err("err", err).Msg("")
		return nil, err
	}
	return
}
...
func (s *Session) request(method, url string, data interface{}, sequence int) (response []byte, err error) {
	...
	defer func() {
		...
		if err2 != nil {
			s.Logger.Error().Caller(3).Msg("error closing resp body")
		}
	}()
	...
	s.Logger.Trace().Caller(3).Int("status_code", resp.StatusCode).
	...
	if err != nil {
		s.Logger.Error().Caller(3).Err("err", err).Msg("response unmarshal error")
		return
	}
	if r.Code != 0 {
		s.Logger.Error().Caller(3).Int("code", r.Code).Str("error_msg", r.Message).Msg("api response error")
